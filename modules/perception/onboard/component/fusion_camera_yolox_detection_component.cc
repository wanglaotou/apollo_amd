 ...
 *****************************************************************************/
#include "modules/perception/onboard/component/fusion_camera_yolox_detection_component.h"
...
FusionCameraYoloxDetectionComponent::~FusionCameraYoloxDetectionComponent() {}

bool FusionCameraYoloxDetectionComponent::Init() {
...
void FusionCameraYoloxDetectionComponent::OnReceiveImage(
  ...
  AINFO << "Enter FusionCameraYoloxDetectionComponent::Proc(), "
...
int FusionCameraYoloxDetectionComponent::InitConfig() {
  ...
  if (camera_names_.size() != 2) {
    AERROR << "Now FusionCameraYoloxDetectionComponent only support 2 cameras";
    return cyber::FAIL;
  }
  ...
  std::string format_str = R"(
      FusionCameraYoloxDetectionComponent InitConfig success
      camera_names:    %s, %s
...
int FusionCameraYoloxDetectionComponent::InitSensorInfo() {
  if (camera_names_.size() != 2) {
...
int FusionCameraYoloxDetectionComponent::InitAlgorithmPlugin() {
...
int FusionCameraYoloxDetectionComponent::InitCameraFrames() {
  if (camera_names_.size() != 2) {
    AERROR << "invalid camera_names_.size(): " << camera_names_.size();
    return cyber::FAIL;
  }
...
int FusionCameraYoloxDetectionComponent::InitProjectMatrix() {
...
int FusionCameraYoloxDetectionComponent::InitCameraListeners() {
  for (size_t i = 0; i < camera_names_.size(); ++i) {
    ...
    std::function<void(const ImageMsgType &)> camera_callback =
        std::bind(&FusionCameraYoloxDetectionComponent::OnReceiveImage, this,
...
int FusionCameraYoloxDetectionComponent::InitMotionService() {
  ...
  std::function<void(const MotionServiceMsgType &)> motion_service_callback =
      std::bind(&FusionCameraYoloxDetectionComponent::OnMotionService, this,
                std::placeholders::_1);
...
void FusionCameraYoloxDetectionComponent::OnMotionService(
...
void FusionCameraYoloxDetectionComponent::SetCameraHeightAndPitch() {
  camera_obstacle_pipeline_->SetCameraHeightAndPitch(
      camera_height_map_, name_camera_pitch_angle_diff_map_,
      default_camera_pitch_);
}

int FusionCameraYoloxDetectionComponent::InternalProc(
  ...
  // opencv 
  cv::Mat mat_image(image_height_, image_width_, CV_8UC3,
              const_cast<char *>(in_message->data().data()), in_message->step());
  cv::cvtColor(mat_image, mat_image, cv::COLOR_RGB2BGR);
  // cv::resize(
  //     mat, mat,
  //     cv::Size(static_cast<int>(image_width_ * ImageHandler::kImageScale),
  //              static_cast<int>(image_height_ * ImageHandler::kImageScale)),
  //     0, 0, cv::INTER_LINEAR);
  ...
  //Seg object in image
  cv::Mat image_gray;
  cv::cvtColor(mat_image, image_gray, CV_BGR2GRAY);
  cv::Mat segmented_mat;
  cv::Mat segmented_mat_gray;
  cv::Mat inputSeg = cv::Mat::zeros(120, 208, CV_8UC1);
  cv::Mat blended_mat;

  pyolox->work_all(mat_image,DetectiontRects,segmented_mat_gray, inputSeg);

  cv::resize(inputSeg, inputSeg, cv::Size(mat_image.cols, mat_image.rows), cv::INTER_NEAREST);

  float scale_width = 1.0 * mat_image.cols / 832;   //1280
  float scale_height = 1.0 * mat_image.rows / 468;  //720

  for(size_t i=0;i<DetectiontRects.size();i++)
  {
    RectClassScore<float> detection;
    detection.x = round(DetectiontRects[i].left * scale_width);
    detection.y = round(DetectiontRects[i].top * scale_height);
    detection.w = round((DetectiontRects[i].right-DetectiontRects[i].left) * scale_width);
    detection.h = round((DetectiontRects[i].bottom-DetectiontRects[i].top) * scale_height);
    detection.classnum = DetectiontRects[i].classnum;
    detection.score = DetectiontRects[i].score;
    std::cout<<"detection info="<<detection.x<<", "<<detection.y<<", "<<detection.w<<", "<<detection.h<<","<<detection.classnum<<","<<detection.score<<std::endl;

    detection.enabled = true;
    if(detection.classnum>0 && detection.classnum<5)
    {
...
int FusionCameraYoloxDetectionComponent::MakeProtobufMsg(
...
int FusionCameraYoloxDetectionComponent::ConvertObjectToPb(
...
int FusionCameraYoloxDetectionComponent::ConvertObjectToCameraObstacle(
    const base::ObjectPtr &object_ptr,
...
int FusionCameraYoloxDetectionComponent::ConvertLaneToCameraLaneline(
...
int FusionCameraYoloxDetectionComponent::MakeCameraDebugMsg(
