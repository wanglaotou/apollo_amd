...
public:
    ...
    //! \see getType()
    //!
    virtual void setType(trtDataType type) TRTNOEXCEPT = 0;
    ...
    //! \see setType()
    //!
    virtual trtDataType getType() const TRTNOEXCEPT = 0;
...
public:
    ...
    virtual void setPrecision(trtDataType trtdataType) TRTNOEXCEPT = 0;
    ...
    virtual trtDataType getPrecision() const TRTNOEXCEPT = 0;
    ...
    //! operations must be trtDataType::kINT32, and all attempts to set the output type to some other data type will be
    ...
    //! \param trtdataType the type of the output
    //!
    //! \see getOutputType() outputTypeIsSet() resetOutputType()

    virtual void setOutputType(int index, trtDataType trtdataType) TRTNOEXCEPT = 0;

    //!
    //! \brief get the output type of this layer
    //!
    //! \param index the index of the output
    //! \return the output precision. If no precision has been set, trtDataType::kFLOAT will be returned,
    //!         unless the output type is inherently trtDataType::kINT32.
    //!
    //! \see getOutputType() outputTypeIsSet() resetOutputType()

    virtual trtDataType getOutputType(int index) const TRTNOEXCEPT = 0;
...
public:
    ...
    //! The #trtDataType for this structure must be ::kFLOAT or ::kHALF, and must be the same
    //! trtdatatype as the input tensor.
    //!
...
public:
    ...
    //!
    //! This tensor must be of type trtDataType::kINT32.
    //!
...
public:
    ...
    //!
    virtual ITensor* addInput(const char* name, trtDataType type, Dims dimensions) TRTNOEXCEPT = 0;
...
extern "C" void* createInferBuilder_INTERNAL(void* logger, int version);
