...
PReLUPlugin::PReLUPlugin(const void *data, size_t length) {
    ...
    read<nvinfer1::trtDataType>(d, mtrtDataType);
    read<int64_t>(d, mWeights.count);
    read<nvinfer1::trtDataType>(d, mWeights.type);
...
size_t PReLUPlugin::getSerializationSize() const {
    return sizeof(mNbInputChannels) + sizeof(mNbInputWidth) + sizeof(mNbInputHeight) + sizeof(mtrtDataType) + 
...
bool PReLUPlugin::supportsFormat(nvinfer1::trtDataType type, nvinfer1::PluginFormat format) const {
    return (type == nvinfer1::trtDataType::kFLOAT || type == nvinfer1::trtDataType::kHALF) 
            && format == nvinfer1::PluginFormat::kNCHW;
}

void PReLUPlugin::configureWithFormat(const nvinfer1::Dims* inputDims, int nbInputs, 
                                      const nvinfer1::Dims* outputDims, int nbOutputs,
                                      nvinfer1::trtDataType type, nvinfer1::PluginFormat format, 
                                      int maxBatchSize) {
    ASSERT((type == nvinfer1::trtDataType::kFLOAT || type == nvinfer1::trtDataType::kHALF)
            && format == nvinfer1::PluginFormat::kNCHW);
...
int PReLUPlugin::initialize() {
    convertAndCopyToDeivce(mDeviceKernel, mWeights, mtrtDataType);
    return 0;
}
...
int PReLUPlugin::enqueue(int batchSize, const void *const *inputs, void **outputs, void *workspace, cudaStream_t stream)
{
    ...
    if (mtrtDataType == nvinfer1::trtDataType::kFLOAT)
    {
...
nvinfer1::IPluginV2* PReLUPluginCreator::createPlugin(const char* name, const nvinfer1::PluginFieldCollection* fc) {
    ...
    nvinfer1::Weights weights{nvinfer1::trtDataType::kFLOAT, weightValues.data(), (int64_t)weightValues.size()};
