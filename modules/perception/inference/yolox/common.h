...
makeObjGuard(T_* t)
{
    YOLOX_CHECK(!(std::is_base_of<T, T_>::value || std::is_same<T, T_>::value));
...
public:
    ...
    trtDataType type() const noexcept override
    {
        return mType;
    }

protected:
    HostMemory(std::size_t size, trtDataType type)
        : mSize(size)
        , mType(type)
    {
    }
...
template <typename ElemType, trtDataType trtdataType>
class TypedHostMemory : public HostMemory
{
public:
    TypedHostMemory(std::size_t size)
        : HostMemory(size, trtdataType)
    {
...
using FloatMemory = TypedHostMemory<float, trtDataType::kFLOAT>;
using HalfMemory = TypedHostMemory<uint16_t, trtDataType::kHALF>;
using ByteMemory = TypedHostMemory<uint8_t, trtDataType::kINT8>;

inline void* safeCudaMalloc(size_t memSize)
{
    void* deviceMem;
    YOLOX_CHECK(cudaMalloc(&deviceMem, memSize));
...
inline unsigned int getElementSize(nvinfer1::trtDataType t)
{
    switch (t)
    {
    case nvinfer1::trtDataType::kINT32: return 4;
    case nvinfer1::trtDataType::kFLOAT: return 4;
    case nvinfer1::trtDataType::kHALF: return 2;
    case nvinfer1::trtDataType::kBOOL:
    case nvinfer1::trtDataType::kINT8: return 1;
    }
    throw std::runtime_error("Invalid trtDataType.");
    return 0;
}
...
inline unsigned int elementSize(trtDataType t)
{
    switch (t)
    {
    case trtDataType::kINT32:
    case trtDataType::kFLOAT: return 4;
    case trtDataType::kHALF: return 2;
    case trtDataType::kBOOL:
    case trtDataType::kINT8: return 1;
    }
    return 0;
}
...
public:
    GpuTimer(cudaStream_t stream)
        : mStream(stream)
    {
        YOLOX_CHECK(cudaEventCreate(&mStart));
        YOLOX_CHECK(cudaEventCreate(&mStop));
    }
    ~GpuTimer()
    {
        YOLOX_CHECK(cudaEventDestroy(mStart));
        YOLOX_CHECK(cudaEventDestroy(mStop));
    }
    void start()
    {
        YOLOX_CHECK(cudaEventRecord(mStart, mStream));
    }
    void stop()
    {
        YOLOX_CHECK(cudaEventRecord(mStop, mStream));
        float ms{0.0f};
        YOLOX_CHECK(cudaEventSynchronize(mStop));
        YOLOX_CHECK(cudaEventElapsedTime(&ms, mStart, mStop));
        mMs += ms;
    }
