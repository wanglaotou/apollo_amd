...
public:
    ...
    //!
    GenericBuffer(nvinfer1::trtDataType type = nvinfer1::trtDataType::kFLOAT)
        : mSize(0)
        , mCapacity(0)
        , mType(type)
        , mBuffer(nullptr)
    ...
    //!
    GenericBuffer(size_t size, nvinfer1::trtDataType type)
        : mSize(size)
        , mCapacity(size)
        , mType(type)
    {
        ...
        , mBuffer(buf.mBuffer)
    {
        buf.mSize = 0;
        buf.mCapacity = 0;
        buf.mType = nvinfer1::trtDataType::kFLOAT;
        buf.mBuffer = nullptr;
    }
...
public:
        ...
        , mBatchSize(batchSize)
    {
        ...
        for (int i = 0; i < mEngine->getNbBindings(); i++)
        {
            ...
            nvinfer1::trtDataType type = mEngine->getBindingtrtDataType(i);
    ...
    void dumpBuffer(std::ostream& os, const std::string& tensorName)
    {
        ...
        for (int i = 0; i < bufDims.nbDims; i++)
            os << ", " << bufDims.d[i];
        os << "]" << std::endl;
        switch (mEngine->getBindingtrtDataType(index))
        {
        case nvinfer1::trtDataType::kINT32: print<int32_t>(os, buf, bufSize, rowCount); break;
        case nvinfer1::trtDataType::kFLOAT: print<float>(os, buf, bufSize, rowCount); break;
        case nvinfer1::trtDataType::kHALF: print<half_float::half>(os, buf, bufSize, rowCount); break;
        case nvinfer1::trtDataType::kINT8: assert(0 && "Int8 network-level input and output is not supported"); break;
        case nvinfer1::trtDataType::kBOOL: assert(0 && "Bool network-level input and output are not supported"); break;
        }
    }
...
private:
    ...
    void memcpyBuffers(const bool copyInput, const bool deviceToHost, const bool async, const cudaStream_t& stream = 0)
    {
        for (int i = 0; i < mEngine->getNbBindings(); i++)
        {
            ...
            {
                if (async)
                    YOLOX_CHECK(cudaMemcpyAsync(dstPtr, srcPtr, byteSize, memcpyType, stream));
                else
                    YOLOX_CHECK(cudaMemcpy(dstPtr, srcPtr, byteSize, memcpyType));
            }
        }
    }
