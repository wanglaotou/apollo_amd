...
template <>
constexpr inline int EnumMax<trtDataType>()
{
    return 5;
} //!< Maximum number of elements in trtDataType enum. \see trtDataType
...
//!
//! Scale is only valid when data type is trtDataType::kINT8. TensorRT will set
...
public:
    ...
    //!
    //! \param type trtDataType requested.
    ...
    //!
    virtual bool supportsFormat(trtDataType type, PluginFormat format) const TRTNOEXCEPT = 0;
    ...
    //! or PluginV2DynamicExt for other PluginFormats.
    //!
    virtual void configureWithFormat(const Dims* inputDims, int nbInputs, const Dims* outputDims, int nbOutputs, trtDataType type, PluginFormat format, int maxBatchSize) TRTNOEXCEPT = 0;
...
public:
    //!
    //! \brief Return the trtDataType of the plugin output at the requested index.
    //! The default behavior should be to return the type of the first input, or trtDataType::kFLOAT if the layer has no inputs.
    //! The returned data type must have a format that is supported by the plugin.
    //! \see supportsFormat()
    //!
    virtual nvinfer1::trtDataType getOutputtrtDataType(int index, const nvinfer1::trtDataType* inputTypes, int nbInputs) const TRTNOEXCEPT = 0;
    ...
    virtual void configurePlugin(const Dims* inputDims, int nbInputs, const Dims* outputDims,
                                 int nbOutputs, const trtDataType* inputTypes, const trtDataType* outputTypes,
...
protected:
    ...
    void configureWithFormat(const Dims* /*inputDims*/, int /*nbInputs*/, const Dims* /*outputDims*/,
                             int /*nbOutputs*/, trtDataType /*type*/, PluginFormat /*format*/, int /*maxBatchSize*/) _TENSORRT_OVERRIDE TRTNOEXCEPT {}
};
...
protected:
    ...
    void configureWithFormat(
        const Dims*, int, const Dims*, int, trtDataType, PluginFormat, int) _TENSORRT_OVERRIDE _TENSORRT_FINAL
    {
    }
    ...
    //!
    TRT_DEPRECATED
    void configurePlugin(const Dims*, int, const Dims*, int, const trtDataType*, const trtDataType*, const bool*, const bool*,
    ...
    //!
    TRT_DEPRECATED
    bool supportsFormat(trtDataType, PluginFormat) const _TENSORRT_OVERRIDE _TENSORRT_FINAL
    {
        return false;
    }
};
...
extern "C" void* createSafeInferRuntime_INTERNAL(void* logger, int version);
